...
// 指定位置へseekし、映像フレームをI420バッファへ直接変換する。
// planar YUV420出力のseek decodeをデコーダ本体から分離する。
...
{$IFDEF DEBUG}
            if DECODE_TRACE_ENABLED then
              DecodeTrace(Format(
                'seek_decode_i420 file="%s" pos_ms=%d target_ts=%d ' +
...
{$IFDEF DEBUG}
    if DECODE_TRACE_ENABLED then
      DecodeTrace(Format(
        'seek_decode_i420_failed file="%s" pos_ms=%d target_ts=%d ' +
